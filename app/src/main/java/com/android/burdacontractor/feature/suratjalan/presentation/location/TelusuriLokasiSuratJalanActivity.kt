...
import com.android.burdacontractor.databinding.ActivityTelusuriLokasiSuratJalanBinding
...
@AndroidEntryPoint
class TelusuriLokasiSuratJalanActivity : AppCompatActivity(), MapEventsReceiver, LocationListener,
    SensorEventListener {
    private lateinit var binding: ActivityTelusuriLokasiSuratJalanBinding
    private val telusuriLokasiSuratJalanViewModel: TelusuriLokasiSuratJalanViewModel by viewModels()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityTelusuriLokasiSuratJalanBinding.inflate(layoutInflater)
        setContentView(binding.root)
        suratJalan = intent.parcelable(Constant.INTENT_PARCEL)
        initMap()
        ...
        telusuriLokasiSuratJalanViewModel.liveNetworkChecker.observe(this) {
            checkConnection(snackbar, it) { initObserver() }
        }
    }

    private fun initObserver() {
        telusuriLokasiSuratJalanViewModel.state.observe(this) {
            when (it) {
        ...
        telusuriLokasiSuratJalanViewModel.distanceMatrix.observe(this) {
    ...
    private fun initMap() {
        ...
        if (suratJalan != null) {
            val desLat = suratJalan!!.tempatTujuan.coordinate.getLatitude()
            val desLong = suratJalan!!.tempatTujuan.coordinate.getLongitude()
    ...
    private fun initUi() {
        with(binding) {
            tvTrackingTo.text = getString(R.string.rute_menuju, suratJalan!!.tempatTujuan?.nama)
            tvNamaDriver.text = suratJalan!!.logistic.nama
            tvNamaSiteManager.text = suratJalan!!.siteManager!!.nama
            tvRoleSiteManager.text = enumValueToNormal(suratJalan!!.siteManager!!.role)
            ivDriver.setImageFromUrl(
                suratJalan!!.logistic.foto,
                this@TelusuriLokasiSuratJalanActivity
            )
    ...
    private fun changeTrackingTo(menuju: String) {
        with(binding) {
            ...
            telusuriLokasiSuratJalanViewModel.getDistanceMatrix(getCoordinate(origin, dest))
    ...
    private fun updateItineraryMarker(
        ...
        marker.setOnMarkerClickListener { mrkr, _ ->
            destinationPoint = mrkr.position
            mrkr.showInfoWindow()
            changeTrackingTo(getString(R.string.rute_menuju, title))
            true
        }
        map.invalidate()
        return marker
    ...
    private fun updateUIWithItineraryMarkers() {
        ...
        suratJalan?.let {
            it.tempatAsal.forEach { tempat ->
                val originLat = tempat.coordinate.getLatitude()
                val originLong = tempat.coordinate.getLongitude()
                val originPoint = GeoPoint(originLat, originLong)
        ...
        markerDestination = updateItineraryMarker(
            ...
            suratJalan!!.tempatTujuan.nama,
            R.drawable.ic_tujuan,
            suratJalan!!.tempatTujuan.alamat
        )
    }
