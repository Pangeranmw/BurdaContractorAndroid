...
@AndroidEntryPoint
class AddPreOrderFragment : BottomSheetDialogFragment() {
    ...
    private fun initUi() {
        with(addDeliveryOrderViewModel) {
            with(binding) {
                ...
                if (preOrderId != null) {
                    val selectedPo =
                        addDeliveryOrderViewModel.listPreOrder.value!!.find { it.id == preOrderId } as PreOrder
    ...
    companion object {
        val TAG = AddPreOrderFragment::class.java.simpleName
        fun newInstance(preOrderId: String? = null, listener: () -> Unit): AddPreOrderFragment {
            val fragment = AddPreOrderFragment()
            if (preOrderId != null) {
