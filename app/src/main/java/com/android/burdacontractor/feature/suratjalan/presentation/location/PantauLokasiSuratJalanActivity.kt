...
class PantauLokasiSuratJalanActivity : AppCompatActivity(), MapEventsReceiver, LocationListener,
    ...
    private val pantauLokasiSuratJalanViewModel: PantauLokasiSuratJalanViewModel by viewModels()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pantauLokasiSuratJalanViewModel.liveNetworkChecker.observe(this) {
            checkConnection(snackbar, it) { initObserver() }
        }
    }

    private fun initObserver() {
        pantauLokasiSuratJalanViewModel.state.observe(this) {
            binding.progressBar.isVisible = it == StateResponse.LOADING
        }
        pantauLokasiSuratJalanViewModel.distanceMatrix.observe(this) {
        ...
        pantauLokasiSuratJalanViewModel.getCoordinate(suratJalan!!.logistic.id)
        pantauLokasiSuratJalanViewModel.logisticCoordinate.observe(this) {
            if (it != null) {
    ...
    private fun initMap() {
        ...
        if (suratJalan != null) {
            val startLat = suratJalan!!.tempatAsal[0].coordinate.getLatitude()
            val startLong = suratJalan!!.tempatAsal[0].coordinate.getLongitude()
    ...
    private fun updateUIWithItineraryMarkers() {
        ...
        suratJalan!!.tempatAsal.forEach {
            val lat = it.coordinate.getLatitude()
            val long = it.coordinate.getLongitude()
    ...
    private fun onRealtimeLocationChanged(pLoc: LogisticCoordinate) {
        ...
        pantauLokasiSuratJalanViewModel.getDistanceMatrix(getCoordinate(origin, dest))
